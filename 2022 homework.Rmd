...
## Week 5 homework
Covering chapters 10 and 11 of the book (lectures 9 and 10).
...
```{r 5.1, fig.dim = c(2, 1.5)}
#load data
data(NWOGrants)
d<-NWOGrants
head(d)
#create numeric variable for gender (female == 2, male ==2)
d$G <- ifelse(d$gender=="f", 1, 2)
...
coordinates(dag) <- list(x = c(awards = 3, discipline = 2, gender = 1),
                      y = c(awards = 1, discipline = 0, gender = 1))

plot(dag, cex = 1.5, lwd = 3)
```

```{r 5.1 model}

#There is a backdoor path from gender through discipline to awards, so for the total effect of gender, don't condition on discipline
#We need a binomial glm for the proportion of awards given the number of applications
...
#to get back on the probability scale from the logit scale, use inv_logit
fpost <- inv_logit(post$a[,1])
mpost <- inv_logit(post$a[,2])

dens(fpost, col = "red", xlim = c(0.1, 0.22), main = "posterior prob of female (red) and male (blue) applicants being accepted")
dens(mpost, col = "blue", add = TRUE)

#calculate the contrast between males and females
diff_prob <- fpost - mpost
dens(diff_prob, main = "0 = no difference, <0 = males advantaged, >0 = females advantaged",
     xlab = "female prob - male prob")

```

The model suggests males are slightly advantaged in their applications, with a man's application having a probability of being accepted between `r abs(PI(diff_prob)[[1]])` and `r abs(PI(diff_prob)[[2]])` (89% interval) greater than a woman's application.


### Q2. 
Now estimate the DIRECT causal effect of gender on grant awards. Compute
...
```{r 5.2}
#To estimate the direct causal effect of Gender, we need to condition on Discipline

#first convert discipline to numeric
d$D <- as.numeric(d$discipline)
...
pA<-inv_logit(post$a)
ndisc<-dim(pA)[3]
post_diff <- sapply(1:ndisc, function(i) pA[,1,i] - pA[,2,i])
#plot (weighting the lines by the proportion of applications per dept)
w <- sapply(1:ndisc, function(i) sum(d$applications[d$D==i])/sum(d$applications))
#also by xtabs
#w <- xtabs(d$applications ~ d$D)/sum(d$applications)
plot(NULL, xlim = c(-0.5, 0.5), ylim = c(0, 20), xlab = "men advantaged <- Gender contrast -> women advantaged", ylab = "Density")
for(i in 1:dim(post_diff)[2]) dens(post_diff[,i], lwd = 2+20*w[i], col = i+1, add = TRUE)
abline(v = 0, lty = 3)
legend("topleft", legend=levels(d$discipline), pch=16, col=1:ndisc+1)

#Finally plot the average causal effect of gender marginalised over departments
#simulate dataset where applications appear the same number of times per discipline 
#as in the original dataset, but where all applications came from women or men
total_apps <- sum(d$applications)
apps_per_D <- sapply(1:ndisc, function(i) sum(d$applications[d$D==i]))
#use link to simulate the log odds of each application/row in the dataset of all women applicants
#(weighted by the number of applications per discipline)
p_women <- link(m5.2, data = list(
  applications = rep(1, total_apps),
  D = rep(1:ndisc, times = apps_per_D),
  G = rep(1, total_apps)
))
#dim(p_women): 2000 2823
#(why 2000 samples when the default is 1000?)
p_men <- link(m5.2, data = list(
  applications = rep(1, total_apps),
  D = rep(1:ndisc, times = apps_per_D),
  G = rep(2, total_apps)
))

p_diff <- inv_logit(p_women) - inv_logit(p_men)

dens(p_diff, lwd = 4, col = 2, xlab = "men <- effect of gender perception -> women")
abline(v = 0, lty = 3)

```

Marginalising over all disciplines, the peak of the probability mass of the difference between the probability of a woman's application being accepted and a man's application being accepted shows a slight advantage for men.  
...
The total effect of gender is that men tend to have a slight advantage over women in their applications. Conditioning on discipline, this advantage is still apparent. Probably because men are more likely to be accepted by departments that receive a larger number of applications.

### 4-OPTIONAL CHALLENGE. 
The data in data(UFClefties) are the outcomes
of 205 Ultimate Fighting Championship (UFC) matches (see ?UFClefties for details). It is widely believed that left-handed fighters (aka “Southpaws”) have an advantage against right-handed fighters, and left-handed men are indeed over-represented among fighters (and fencers and tennis players) compared to the general population. Estimate the average advantage, if any, that a left-handed fighter has
against right-handed fighters. Based upon your estimate, why do you think left-handers are over-represented among UFC fighters?
...
data("UFClefties")
d <- UFClefties
#model fighter1.win ~ fighter1.lefty*fighter2.lefty (because lefites have advantage over righties only?)
#in McElreath's code, that is a matrix of figher1.lefty and fighter2.lefty
#Make fighter1.lefty and fighter2.lefty into a handedness index, 1 = R, 2 = L
#(also change fighter1.win to win, and remove other cols cos stan can't deal with "." in colnames)
d <- d %>% mutate(win = fighter1.win, H1 = fighter1.lefty+1, H2 = fighter2.lefty+1) %>% 
  select(H1, H2, win)
...
# trankplot(m5.4)
precis(m5.4, depth = 3)
#diagnsotics look good

#plot the estimates
post <- extract.samples(m5.4)
p <- inv_logit(post$a) #dim = [sample, winner_handedness, opponent_handedness]
#Q do lefties have an advantage when fighting righties v when fighting lefties?
#difference between prob of lefty beating righty and lefty beating lefty
pdiff1 <- p[,1,1]-p[,1,2] #(p[lefties-lefties] - p[lefties-righties])
dens(pdiff1, col = 2, lwd = 4, xlab = "adv against righty <- prob lefty wins -> adv against lefty")
abline(v = 0, lty = 3)
#A lefty is slightly more likely to win against another lefty than against a righty

#Q what's the average difference between lefties and righties marginalised over opponent handedness?
#(what would be the effect of changing fighter 1's handedness in a fight?)
#(weighing fights by the number of left and right handed opponents)
nfights<-length(d$H1)
nLRopp<-sapply(1:2, function(i) length(d$H2[d$H2==i]))
pL<-link(m5.4, data = list(
  H1 = rep(1, nfights),
  H2 = rep(1:2, times = nLRopp)
))
pR<-link(m5.4, data = list(
  H1 = rep(2, nfights),
  H2 = rep(1:2, times = nLRopp)
))
...
dens(pdiff2, col = 2, lwd = 4, xlab = "right adv <- effect of handedness -> left adv")

# 
# The plots show that lefties are more likely to win a fight, but also that they have more of an advantage over other lefties than over righties. Possibly there are a couple of great lefty fighters skewing the results in the lefties favour - a random effect of fighter ID could account for this.
# Lefties might be overrepresented due to the reputations of a small number of very good lefty fighters racking up a lot of wins, making it seem like being a lefty is an advantage.
...
#The dataset is dyadic, in that each row shows two fighters.
#We need to model this in a way that is invariant to fighter position,
#so if there is an advantage for lefties, then:
#prob_win1 ~ left_adv*fighter1.lefty - left_adv*fighter2.lefty
#prob_win1 ~ left_adv(fighter1.lefty - fighter2.lefty) #left_adv is same regardless of fighter

data("UFClefties")
d<-UFClefties %>% select(win = fighter1.win, L1 = fighter1.lefty, L2 = fighter2.lefty)
m5.4s <- ulam(
  alist(
    win ~ dbinom(1, p),
    logit(p) <- a + bL*(L1-L2),
...
#plot estimates
post<-extract.samples(m5.4s)
dens(inv_logit(post$a + post$bL), lwd = 4, col =2, main = "Southpaw advantage?", xlab = "prob of lefty winning")
abline(v = 0.5, lty = 3)

```

No evidence of a lefty advantage.  
So why are there so many lefties in boxing (and tennis and fencing)? McElreath suggests one possible answer is that there really is a lefty advantage but by the time they make it to professional levels, there is a collider bias, where both ability and left-handedness influence qualifying to pro levels, so right-handers at pro levels have greater ability on average to make up for the lefty advantage, meaning the lefy advantage is meaningless at pro level.
We could account for ability levels by including fighter ID as a random effect (but we haven't covered mixed models yet).  


## Week 6 homework
Covering chapters 12 and 13 of the book (lectures 11 and 12).
Ordered categories and multilevel (mixed) models.  

The theme of this homework is tadpoles. You must keep them alive.

### Q1. 
1. Conduct a prior predictive simulation for the Reedfrog model. By this I mean to
simulate the prior distribution of tank survival probabilities αj. Start by using this
prior:

$$αj ∼Normal(\overline{α}, σ)\\
\overline{α} ∼Normal(0, 1)\\
σ ∼Exponential(1)$$  
...
sigma <- rexp(N, 1)
aj<-rnorm(N, a_, sigma)
aj_prob <- inv_logit(aj)
dens(aj_prob, main = "prior predictive probability of tank survival, sigma ~ exp(1)")
#sigma ~ exp(10)
sigma <- rexp(N, 10)
aj<-rnorm(N, a_, sigma)
aj_prob <- inv_logit(aj)
dens(aj_prob, main = "prior predictive probability of tank survival, sigma ~ exp(10)")
#sigma ~ exp(0.1)
sigma <- rexp(N, 0.1)
aj<-rnorm(N, a_, sigma)
aj_prob <- inv_logit(aj)
dens(aj_prob, main = "prior predictive probability of tank survival, sigma ~ exp(0.1)")

```

Increasing the width of the prior on sigma pushes more of the prior probability to the middle of the distribution (i.e. giving the most weight to survival probabilities around 0.5), while using a narrower prior on sigma forces the probability mass to the extreme values of 0 and 1, meaning we are telling the model to expect that either all tadpoles die or all survive in a given tank.  


### Q2. 
Revisit the Reedfrog survival data, data(reedfrogs). Start with the varying
effects model *(I think "varying effects" is just the same as "random effects"?)* from the book and lecture. Then modify it to estimate the causal effects
...
#Varying/Random effects model
#first, make the tank cluster variable (each row is a different tank)
d$tank <- 1:nrow(d)
d$P <- ifelse(d$pred=="no", 1, 2)
d$big <- ifelse(d$size=="big", 1, 2)

dat <- list(N = d$density,
            S = d$surv,
            tank = d$tank,
            P = d$P,
            big = d$big)
#Varying effects model 
# (All that is required to enable adaptive pooling is to make the prior for the a parameters a func-
# tion of some new parameters.)
#  m6.2var<-ulam(
#    alist(
#      S ~ dbinom(N, p),
#      logit(p) <- a[tank],
...
#  )
# precis(m6.2var, depth = 2)

#Now modify the model to include the effects of pred and size, including how size might modify the effect of pred (so a random intercept for tank, and an interaction between pred and size?)
#Can't get the interaction to work - using the hint to estimate an effect for each combination of pred and size
# d$ps <- factor(paste(d$pred, d$size))
# d$ps1 <-as.numeric(d$ps)
# dat$PS <- d$ps1
...
#  )
# precis(m6.2, depth = 2)
# #neff is quite low for b - problems estimating?
# #compare(m6.2var, m6.2)
# 
...
#This model is overpredicting survival for many tanks - predictions are all in the range 0.7-0.99, while prop_surv is between 0.1 and 1.
#Why? 
#Because I'm adding the slope to the tank mean survival? Should I be adding it to the overall sample (average) intercept (a_bar)? Another linear model for a_bar (a model within a model)?
#No - it was because I wasn't (a) specifying the model correctly and (b) I wasn't calculating the predictions properly.
...
#Try again:
 ...
 m6.2a<-ulam(
   alist(
     S ~ dbinom(N, p),
     logit(p) <- a[tank] + b[P, big],
     #a[tank] ~ dnorm(a_bar, sigma) ,
     #a_bar ~ dnorm(0, 1.5),
     a[tank] ~ dnorm(0, sigma), #McElreath sets the mean to 0 rather than learning from the data - why?
     sigma ~ dexp(1),
     matrix[P, big]:b ~ dnorm(0, 1)
   ), data = dat, chains = 4, log_lik = T
 )
precis(m6.2a, depth = 3, pars = c("b", "sigma"))
#seems to run ok, with good rhat and n_eff values

#plot the predictions
# extract Stan samples
post <- extract.samples(m6.2a)
# compute mean intercept for each tank
# also transform to probability with logistic
d$tank.est <- apply( post$a , 2 , mean )
d <- d %>% mutate(beta.est = case_when(
  P==1 & big==1 ~ mean(post$b[,1,1]),
  P==1 & big==2 ~ mean(post$b[,1,2]),
  P==2 & big==1 ~ mean(post$b[,2,1]),
  TRUE ~ mean(post$b[,2,2])),
  propsurv.est = logistic(tank.est + beta.est))
# display raw proportions surviving in each tank
plot( d$propsurv , ylim=c(0,1) , pch=16 , xaxt="n" ,
xlab="tank" , ylab="proportion survival" , col=rangi2 )
axis( 1 , at=c(1,16,32,48) , labels=c(1,16,32,48) )
...
You can see in this plot the effects of the regularising prior learned from the data. Predictions for more extreme values are closer to the overall mean value for all tanks due to shrinkage.  


```{r 6.2 effects}

#so what is the model saying that the effect of predation is on big and small tadpoles?
#calculate the expected mean value for each tank under different interventions (big, small, pred, no pred)
abar<-d$tank.est
beta <-c()
for(i in 1:2) for(j in 1:2) beta<-c(beta, mean(post$b[,i,j]))
cols <- c("black", "black", "red", "red")
lins <- c(1, 2, 1, 2)
plot(NULL, xlim = c(0, 1), ylim = c(0, 18), main ="survival probability", xlab = "prob", ylab = "density")
for(i in 1:4) dens(logistic(abar + beta[i]), add = T, col = cols[i], lty = lins[i])
legend("topleft", c("no predators, big", "no predators, small", 
                    "predators, big", "predators, small"),
       lty = lins,
       col = cols)
...
Without predators, survival probability is pretty similar regardless of the size of the tadpoles (although there is slightly higher survival probability for bigger tadpoles). 
However, with predators, survival probability is lower for bigger tadpoles compared to smaller tadpoles.

Dag of the experiment:
```{r 6.2 dag,  fig.dim = c(2, 1.5)}
survdag<-dagitty( "dag{
  predators -> survival <- size; tank -> survival
}"
)
...
your Binomial GLM. You could treat it as a continuous regression variable (possibly standardized). Or you could convert it to an ordered category (with three levels). Compare the σ (tank standard deviation) posterior distribution to σ from your model in Problem 2. How are they different? Why?

```{r 6.3}
#Can I use density in the model twice? Once for the N in dbinom and once as a predictor variable?
#Or shoudl I use propsurv instead of surv and N?

#Again, I misunderstood this question - it wants us to include density in the previous model, not to just estimate the causal effects of density. See below for correct answer.

#Try with density twice
#Using an ordered category for density
d$dens<-as.numeric(factor(d$density))
dat$D <- d$dens
dat$D_log<-standardize(log(dat$D)) #use log to account for diminishing returns on log-odds scale

# m6.3 <- ulam(
#   alist(
#     S ~ dbinom(N, p),
#     logit(p) <- a[tank] + b[P, D],
#     a[tank] ~ dnorm(0, sigma),
#     matrix[P, D]:b ~ dnorm(0, 1),
#     sigma ~ dexp(1)
#   ), data = dat, chains = 4, log_lik = T
# )
# precis(m6.3, depth = 3, pars = c("b", "sigma"))
# 
# #plot
# post<-extract.samples(m6.3)
# cols = c(rep("black", 3), rep("red",3))
# lins = c(1, 2, 3, 1, 2, 3)
# plot(NULL, xlab = "prob surv", ylab = "density", xlim = c(0, 1), ylim = c(0, 20))
# for(i in 1:6) dens(logistic(post$alpha + post$b[,i]), col = cols[i], lwd = lins[i], add = T)
# legend("topleft", c("no predators, low density", "no predators, mid density", "no predators, high density",
#                     "predators, low density", "predators, mid density", "predators, high density"),
#        lwd = lins,
#        col = cols)
...
m6.3 <- ulam(
  alist(
    S ~ dbinom(N, p),
    logit(p) <- a[tank] + b[P, big] + bD[P]*D_log,
    a[tank] ~ dnorm(0, sigma),
    matrix[P, big]:b ~ dnorm(0, 1),
    bD[P] ~ dnorm(0, 0.5),
    sigma ~ dexp(1)
...
precis(m6.3, depth = 3, pars = c("b", "bD", "sigma"))

#plot
nrows<-48*12
plotdat<-data.frame(tank = rep(1:48, each = 12), 
              P = rep(c(1,1,2,2), nrows/4), 
              big = rep(c(1,2,1,2), nrows/4),
              D = rep(1:3, each = 4, times = nrows/12),
              N = rep(c(10,25,35), each = 4, times = nrows/12),
              combination = rep(1:12, nrows/12))
plotdat$D_log <- standardize(log(plotdat$D))
simdat<-list()
#for(i in 1:12) simdat[[i]] <- logistic(sim(m6.3, data = plotdat[plotdat$combination==i,]))
for(i in 1:12) simdat[[i]] <- link(m6.3, data = plotdat[plotdat$combination==i,])


cols <- rep(c("black", "red"), each = 2, times = 3)
lwds <- rep(c(2, 1), times = 6)
ltys <- rep(c(3, 2, 1), each = 4)
plot(NULL, xlim = c(0, 1), ylim = c(0, 12), main ="survival probability", xlab = "prob", ylab = "density")
for(i in 1:12) dens(simdat[[i]], add = T, col = cols[i], lty = ltys[i], lwd = lwds[i])
legend("topleft", c("no predators", "predators"), lty = 1, col = c("black", "red"))
legend(x = -0.04, y = 9.95, c("low density", "mid density", "high density"), lty = c(3, 2, 1))
legend(x = -0.04, y = 6.55, c("small", "big"), lwd = c(1, 2))


```

This model suggests that, without predators, there is not much difference in the probability of survival between the three different levels of density, although size and density do have an effect on survival even in the absence of predation. With predators, however, survival probability is greatly reduced, particularly for large tadpoles in high density and mid density tanks, with predicted survival much greater for small tadpoles in low density tanks (perhaps because there are more places to hide from predators?).

Sigma from this model (89% range `r precis(m6.3, par = "sigma")[3,1]` - `r precis(m6.3, par = "sigma")[4,1]`) is lower than sigma from the previous model (89% range `r precis(m6.2a, par = "sigma")[3,1]` - `r precis(m6.2a, par = "sigma")[4,1]`), presumably because the addition of density as a predictor in the second model has mopped up some of the unexplained variance.

### Optional Q4.  
Using your estimates from the previous problems,
compute the expected causal effect of removing predators from a population of tad-
poles with size and density distributed according to this table:
...
*I don't think I am understanding this question properly. Calculate the predicted survival rate in this population based on the effects of density and size without predators, using the estimates from the previous two models (not the previous 1 model)? Is it asking for the expected population distribution at the end of the experiment?*

*Yeah, after looking at the solutions, I didn't understand this question at all. What it wants is the difference between these 4 tanks with predators and without predators. Solution below.*

```{r 6.4, fig.show="hold", out.width="33.3333%"}

#First simulate the population in (100) different tanks and model predictions with predation
nreps <- 25
D<-rep(c(10,35,10,35), nreps)
D_log<-(D-mean(d$density))/sd(d$density)  #standardise D as in the model
big<-rep(c(1,1,2,2), nreps)
P<-rep(rep(2, 4), nreps) #all predators
#Generate tank intercepts for our simulated tanks (200 values each) based on the sigma values from the model
post<-extract.samples(m6.3)
aT<-replicate(length(D), rnorm(2000, 0, post$sigma)) #2000 samples for each of our 100 tanks
#Now simulate predicted distribution of survival for each tank under predation
...
#Next change predation (P) to all 1s (ie. no predators)
P<- rep(rep(1, 4), nreps)
#And simulated predicted distribution of survival for each tank without predation
...
#find the difference that removing predators makes by subtracting P2 from P1
P3<-P2-P1

#plot predicted survival with and without predation
Ps<-list(P1, P2, P3)
mains <- c("survival with predation", "survival without predation", "effect of removing predators")
xlabs<-c("survival prob", "survival prob", "change in survival prob")
for(i in 1:3) dens(Ps[[i]], main = mains[i], xlab = xlabs[i], ylab = "density")

```

"The intervention (*removing predators*) helps a lot, but because the survival varies so much under status
quo (*with predation*), the causal effect ranges from almost nothing to at most 80%."
